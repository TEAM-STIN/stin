...
에이전트는 레포에 **이미 있는 패턴을 복제한다.** 최적이 아닌 패턴도 그대로 번식하므로
드리프트는 막을 수 없고, 쌓이기 전에 걷어내는 수밖에 없다.

사람이 몰아서 치우는 방식은 2인 팀에서 확장되지 않는다. 그래서 기계가 먼저 뽑는다.

```bash
pnpm gardening    # 언제든 직접 실행
```

매주 월요일 아침 CI가 같은 점검을 돌리고, 정리할 것이 있으면 `gardening` 라벨이
붙은 이슈를 연다. **이미 열린 점검 이슈가 있으면 새로 만들지 않고 댓글만 단다** —
이슈가 쌓이면 아무도 안 보게 되고, 그 순간 이 점검은 배경 소음이 된다.

### 점검하는 것

| 항목 | 왜 |
|---|---|
| 30일 넘게 방치된 기술 부채 | 안 할 거면 지운다. 남겨두기만 하면 목록이 소음이 된다 |
| 완료됐는데 `active/`에 남은 실행 계획 | `active/`가 실제 진행 중인 것만 담아야 의미가 있다 |
| `AGENTS.md`가 상한(100줄)에 근접 | 넘긴 뒤 급하게 쪼개면 구조가 나빠진다 |
| 비어 있는 `docs/references/` | 없으면 에이전트가 존재하지 않는 API를 쓴다 |
| 짝이 되는 테스트가 없는 소스 | 전부 테스트하라는 뜻은 아니다. 판단하라는 뜻이다 |

### 사람이 하는 것

기계가 못 보는 건 사람이 본다. **10분을 넘기지 않는다** — 넘기면 이 점검 자체가
부담이 되어 건너뛰게 된다.

1. 이슈의 항목을 훑고 처리할 것과 지울 것을 정한다
2. `/code-review`를 최근 변경 전체에 한 번 돌린다
3. 나온 지적을 [exec-plans/tech-debt-tracker.md](exec-plans/tech-debt-tracker.md)에 적재한다
4. 같은 지적이 반복되면 아래 승격 규칙을 적용한다
