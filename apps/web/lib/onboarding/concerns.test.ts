...
describe("parseConcerns", () => {
  it("쉼표 문자열을 파싱한다", () => {
    expect(parseConcerns("TROUBLE,PORE")).toEqual(["TROUBLE", "PORE"]);
  });
  ...
  it("잘못된 값과 중복을 버린다", () => {
    expect(parseConcerns("TROUBLE,BOGUS,TROUBLE,PORE")).toEqual(["TROUBLE", "PORE"]);
  });
  ...
  it("순서를 CONCERN_OPTIONS 기준으로 정규화한다", () => {
    ...
    expect(parseConcerns("PIGMENTATION,TROUBLE")).toEqual(["TROUBLE", "PIGMENTATION"]);
  });
});

describe("serializeConcerns", () => {
  it("선택 순서와 무관하게 CONCERN_OPTIONS 순서로 직렬화한다", () => {
    expect(serializeConcerns(["PORE", "TROUBLE"])).toBe("TROUBLE,PORE");
  });
